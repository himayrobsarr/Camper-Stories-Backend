...
const CamperProjectController = {
  ...
  getProjectsByCamperId: async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      res.status(500).json({
        message: "Error al obtener los proyectos del camper",
        error: error.message,
      });
    }
  },
  ...
  addProjectForCamper: async (req, res) => {
    ...
    try {
      ...
      const result = await CamperProjectModel.addProjectForCamper(
        camper_id,
        projectData,
        requestingUserId
      );
  ...
  updateProjectForCamper: async (req, res) => {
    try {
      ...
      const result = await CamperProjectModel.updateProjectForCamper(
        camper_id,
        project_id,
        updates
      );
  ...
  getProjectTechnologies: async (req, res) => {
    ...
    try {
      ...
      const technologies = await CamperProjectModel.getProjectTechnologies(
        projectId
      );

      // Siempre devolver un array, incluso si está vacío
      res.status(200).json({
        message: technologies.length
