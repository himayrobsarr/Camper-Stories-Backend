...
const landingAIModel = {
    saveRegisteredInfo: async ({ name, lastname, email, phone, document, payment_reference, payment_date }) => {
      try {

        const formattedDate = new Date(payment_date).toISOString().slice(0, 19).replace('T', ' ');
  
        const query = `
          INSERT INTO INSCRIPCIONES_IA (name, lastname, email, phone, document, payment_reference, payment_date)
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `;
        const values = [name, lastname, email, phone, document, payment_reference, formattedDate];
